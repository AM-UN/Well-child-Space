...
スクールには、コロナ渦で学校に通えなくなってしまった子、人間関係に悩んでしまった子など、様々なバックグラウンドの子供たちが在籍しています。私は授業の最後やslackのメッセージなどでお母様方と接する機会があるのですが、その中で子育ての大変さを感じる場面が多々ありました。  
<br>
...
# 工夫した点
#### ・issueとプルリクエストを活用した疑似的なチーム開発  
#### ・Docker、DockerComposeを用いた開発環境構築
#### ・AWSを用いてインフラ構築、ALBを経由し常時SSL通信を行っている  
